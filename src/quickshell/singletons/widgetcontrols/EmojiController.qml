...
Item {
    ...
    // Optional requested tab when opening ("emoji" | "nerd" | "kaomoji").
    // Bumping requestNonce lets the widget react even if the same mode is
    // requested twice in a row (e.g. two IPC calls with the same arg).
    ...
    function toggle(scr, mode) {
        ...
        if (controller.isVisible) {
            ...
            } else if (mode !== undefined && mode !== null && mode !== "") {
                controller.requestedMode = mode;
                controller.requestNonce++;
            } else {
                hide();
